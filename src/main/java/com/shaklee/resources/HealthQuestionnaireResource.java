...
public class HealthQuestionnaireResource {
	...
	public HQResponse runQuestionnaire(@RequestBody Questions questions) throws InputValidationException {
		...
		try {
			...
			HealthPrintContentRequest<Questions, Bundle, SKU> req = new HealthPrintContentRequest<Questions, Bundle, SKU>(
					questions, null, null);
			...
			// logActivity("RECOMMENDATIONS","FAILED " + e.toString(), questions,
			// e.toString());
	...
	public StorageResponse update(@RequestBody StorageRequest request, @CurrentUser User user)
