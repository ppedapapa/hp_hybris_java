...
public class PromoJsonLoader {
	...
	public PromoJsonLoader() {
		mapper = JsonLoader.createDefaultMapper();
	...
	public JsonNode serlialize(Object obj) throws JsonProcessingException, IOException {
	...
	public JsonNode deserialize(String json) throws JsonProcessingException {
		try {
			...
			throw new RuntimeException("Unexpected IO error from simple string (which does not require IO)", e);
		}
	}
	...
	public void deserialize(Object obj, Map<String, List<String>> data)
		...
		for (Map.Entry<String, List<String>> e : data.entrySet()) {
			...
			if (schema == null) {
				...
				if (node == null) {
					String msg = "Database parameter '" + name + "' does not exist in class "
							+ obj.getClass().getSimpleName();
					throw InputValidationException.oneFieldError(msg, name, msg);
				}
	...
	public static class SchemaCache {
		...
		private final HashMap<Class<?>, ObjectNode> cache = new HashMap<Class<?>, ObjectNode>();
		private final ObjectMapper mapper;

		public synchronized ObjectNode get(Class<?> clazz) throws JsonMappingException {

			ObjectNode schema = cache.get(clazz);
			if (schema == null) {
				schema = (ObjectNode) mapper.generateJsonSchema(clazz).getSchemaNode().get("properties");
