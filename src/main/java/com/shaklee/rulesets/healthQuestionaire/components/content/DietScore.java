...
@Component
public class DietScore extends AbstractScoreComponent<HealthPrintContentRequest<Questions, Bundle, SKU>>
		implements Action<HealthPrintContentRequest<Questions, Bundle, SKU>> {

	public Map<String, List<Integer>> scores;

	/**
	 * Generates the Diet score based on request (non-Javadoc)
	 * 
	...
	public void exec(HealthPrintContentRequest<Questions, Bundle, SKU> request) throws InputValidationException {
		...
		request.score.diet_score = (int) (calculate(request.request) * bmiFactor(request.score.bmi));
	...
	protected float calculate(Questions questions) {
		...
		return ((float) score / (scores.values().size()
				- (questions.dietary_restrictions.contains(DietaryRestriction.DAIRY) ? 1 : 0)));
	}
}
